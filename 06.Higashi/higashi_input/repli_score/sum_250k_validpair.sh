...
sample="snHiChewHEK293Twt_250k_validpair"
# batchfile=($(ls -d /research/xieyeming1/proj_2023/hichew_paper_20230710/scHiC_benchmark/${sample}/hic_pro/*/|rev|cut -c 2-|rev))
batchfile=($(cat /research/xieyeming1/proj_2023/hichew_paper_20230710/scHiC_benchmark/snHiChew_HEK293Twt/benchmark/hic_pro_metrics_sum/filtered_snHiChew_HEK293Twt_hic_pro_metrics.xls|grep -v sample|awk '$12>500000'|cut -f1))
...
for i in ${sample_id[@]}; do
  ...
  cat tmp/${i}.early_ratio|awk -v a="${i}" -v b="${cell_id}" '{print a"\t"b"\t"$2}' >> ${sample}_label_info.txt
  ((cell_id++))
done
