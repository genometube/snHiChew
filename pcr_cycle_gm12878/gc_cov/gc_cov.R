...
################## gc cov 20241016 ##################
dipC<-read.table('dipC_gc_cov.xls',header=TRUE)
cycle15<-read.table('cycle_15_gc_cov.xls',header=TRUE)
cycle18<-read.table('cycle_18_gc_cov.xls',header=TRUE)
cycle20<-read.table('cycle_20_gc_cov.xls',header=TRUE)

stack<-rbind.data.frame(dipC,cycle15,cycle18,cycle20)
dev.off()

pdf('GC_coverage.pdf',width=6,height=3)
ggplot(stack, aes(x=GC,y=NORMALIZED_COVERAGE,color=cycle_number))+ geom_smooth(lwd=0.5)+theme_classic()+
  scale_color_manual(values=c(mypal[2],mypal[1],mypal[4],mypal[9]))+
  labs(title='GC coverage, N=10')
dev.off()

svg('GC_coverage.svg',width=6,height=3)
ggplot(stack, aes(x=GC,y=NORMALIZED_COVERAGE,color=cycle_number))+ geom_smooth(lwd=0.5)+theme_classic()+
  scale_color_manual(values=c(mypal[2],mypal[1],mypal[4],mypal[9]))+
  labs(title='GC coverage, N=10')
dev.off()
