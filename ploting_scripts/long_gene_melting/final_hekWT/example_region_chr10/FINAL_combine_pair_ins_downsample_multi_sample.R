...
plot_list <- list()
#chr10	131265448	131565884	rna_seq	37.28	4255
...
#end-start
#sample_group<-c('hichew_293t','293Fla','snHiChewTripto')
sample_group<-c('hichew_293t')
validpair='500000'
...
for(i in 1:length(sample_group)){
  ...
  ins_input<-ins_input[,c(-1,-3)]
  colnames(ins_input)<-c('pos','ins','cell')
  ins_input$pos<-ins_input$pos/bin
  #head(ins_input_raw)
  #ead(ins_input)
  ...
  p<-grid.grabExpr(draw(Heatmap(dat_mtx__ins, name = "ins", cluster_rows = color_branches(dend,k=2,col=c(melt_col, conc_col)), row_split = clust_num,
  ...
  ins_clust1<-as.data.frame(ins_clust[ins_clust$clust=='cluster_1',])
  ins_clust2<-as.data.frame(ins_clust[ins_clust$clust=='cluster_2',])
  sorted_ins_clust_1 <- ins_clust1[order(ins_clust1$ins), ]
  sorted_ins_clust_2 <- ins_clust2[order(ins_clust2$ins), ]
  ...
  ks_test<-ks.test(sorted_ins_clust_1$ins,sorted_ins_clust_2$ins)
  ks.test(sorted_ins_clust_1$ins,sorted_ins_clust_2$ins)
  
  d<-round(as.numeric(ks_test[1]),3)
  pval<- round(-log10(as.numeric(ks_test[2])),3)
  
  p3<-ggplot(sorted_ins_clust_b, aes(x = ins, y = cumulative_prob,group=clust,color=clust)) +
...
pdf(paste0('validpair_downsample_sc_ins_cell_',start,'_',end,'.pdf'),height = 15,width = 5)
cowplot::plot_grid(plotlist = plot_list, nrow = 1)
dev.off()

png(paste0('validpair_downsample_sc_ins_cell_',start,'_',end,'.png'),height = 1200,width = 400)
cowplot::plot_grid(plotlist = plot_list, nrow = 1)
dev.off()
