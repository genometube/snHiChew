...
ggplot(stack_df, aes(x = readNum_per_cell, y = mean_rmdup_valid_pair,group=sample,color=sample))+ 
  geom_line()+ geom_point()+geom_pointrange(aes(x=readNum_per_cell, ymin=mean_rmdup_valid_pair-sd, ymax=mean_rmdup_valid_pair+sd),alpha=0.7, size=0.3)+
...
nagano2017<-read.table('sciHiC_downsample_rmdup_validpair.xls',header=FALSE)
nagano2017$sample<-'sciHiC'
colnames(nagano2017)<-c('readNum_per_cell','mean_rmdup_valid_pair','sd','sample')

dip_C<-read.table('zhichao_dipC400cellgm12878_20240930_downsample_rmdup_validpair.xls',header=FALSE)
dip_C$sample<-'dip_C'
colnames(dip_C)<-c('readNum_per_cell','mean_rmdup_valid_pair','sd','sample')

hichew<-read.table('zhichao_snHiChew400cellgm12878_20240918_downsample_rmdup_validpair.xls',header=FALSE)
hichew$sample<-'hichew'
colnames(hichew)<-c('readNum_per_cell','mean_rmdup_valid_pair','sd','sample')

LiMCA<-read.table('LiMCA_downsample_rmdup_validpair.xls',header=FALSE)
LiMCA$sample<-'LiMCA'
colnames(LiMCA)<-c('readNum_per_cell','mean_rmdup_valid_pair','sd','sample')

stack_df<-rbind.data.frame(nagano2017,dip_C,hichew,LiMCA)
#stack_df<-stack_df[stack_df$readNum_per_cell<6400000,]
...
#xlim(0,4000000)+ylim(0,100000)+


######################### 20240505
nagano2017<-read.table('sciHiC_downsample_dup.xls',header=FALSE)
nagano2017$sample<-'sciHiC'
colnames(nagano2017)<-c('readNum_per_cell','rmdvalidpair_dup','sd','sample')

dip_C<-read.table('zhichao_dipC400cellgm12878_20240930_downsample_dup.xls',header=FALSE)
dip_C$sample<-'dip_C'
...
hichew<-read.table('zhichao_snHiChew400cellgm12878_20240918_downsample_dup.xls',header=FALSE)
hichew$sample<-'hichew'
colnames(hichew)<-c('readNum_per_cell','rmdvalidpair_dup','sd','sample')

LiMCA<-read.table('LiMCA_downsample_dup.xls',header=FALSE)
LiMCA$sample<-'LiMCA'
colnames(LiMCA)<-c('readNum_per_cell','rmdvalidpair_dup','sd','sample')
...
p2<-ggplot(stack_df, aes(x = readNum_per_cell, y = rmdvalidpair_dup,group=sample,color=sample))+ 
  geom_line(alpha=0.8,lwd=1,aes(linetype=sample))+ geom_point()+scale_linetype_manual(values=c(3,1,5,2))+
...
library(gridExtra)
pdf('rmd_validpair_saturation_curve_validpair_dup.pdf',height=12,width=7)
