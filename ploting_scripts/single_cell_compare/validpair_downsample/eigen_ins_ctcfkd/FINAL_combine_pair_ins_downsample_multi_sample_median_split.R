...
for(i in 1:length(sample_group)){
  ...
  cell_early_id<-read.table(paste0(sample,'_',validpair,'_','label_info.txt'),sep = '\t')
  ...
  p1<-ggplot(summary_stats_df, aes(x=locus, y = mean, ymin = mean - sd, ymax = mean + sd,color=group)) +
    geom_line(alpha=0.8,lwd=1) + labs(x = "Locus", y = "ins") + theme_classic()+
    scale_color_manual(values=c(melt_col,conc_col))+theme(legend.position = "top")+ylim(-0.5,1.5)
...
pdf(paste0('validpair_downsample_sc_ins_cell_',start,'_',end,'_',clust_method,'.pdf'),height = 18,width = 6)
cowplot::plot_grid(plotlist = plot_list, nrow = 1)
dev.off()

png(paste0('validpair_downsample_sc_ins_cell_',start,'_',end,'_',clust_method,'.png'),height = 1800,width = 600)
cowplot::plot_grid(plotlist = plot_list, nrow = 1)
dev.off()
